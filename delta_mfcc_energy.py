...
def extract_mfcc_features_and_energy(signal, sr, frame_size_ms=30, frame_shift_ms=10, num_ceps=12):
    ...
    # Calculate energy and delta energy for each frame
    energy = np.sum(frames**2, axis=1)
    delta_energy = librosa.feature.delta(energy)
    
    # Calculate mfcc and delta mfcc coefficients
    ...
    return np.c_[mel_cepstral.T[:len(energy), :],  delta_mfcc.T[:len(energy), :], energy, delta_energy]
