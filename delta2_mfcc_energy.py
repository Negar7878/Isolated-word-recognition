...
def extract_mfcc_features_and_energy(signal, sr, frame_size_ms=30, frame_shift_ms=10, num_ceps=12):
    ...
    # Calculate energy, delta energy and delta delta energy for each frame
    energy = np.sum(frames**2, axis=1)
    delta_energy = librosa.feature.delta(energy)
    delta2_energy = librosa.feature.delta(energy, order=2)
    
    # Calculate mfcc, delta mfcc and delta2 mfcc coefficients
    mel_cepstral = librosa.feature.mfcc(y=signal, sr=sr, n_mfcc=num_ceps, hop_length=frame_shift_sample, n_fft=frame_size_sample)
    delta_mfcc = librosa.feature.delta(mel_cepstral)
    delta2_mfcc = librosa.feature.delta(mel_cepstral, order=2)
    
    # Concatenating the features
    return np.c_[mel_cepstral.T[:len(energy), :],  delta_mfcc.T[:len(energy), :], 
                 delta2_mfcc.T[:len(energy), :], energy, delta_energy, delta2_energy]
...
train_mfccs_energies = [extract_mfcc_features_and_energy(train_signal, 16000) for train_signal in train_signals]
...
for test_file in test_files:
    ...
    mfcc_energy_test = extract_mfcc_features_and_energy(resampled_signal_test, 16000)
