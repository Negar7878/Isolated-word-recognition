...
def Wav2Vec2_Features_Extraction(signal):
...
train_wav2vec2_features = [Wav2Vec2_Features_Extraction(train_file) for train_file in train_files]
True_Labels = list(range(10))
Predicted_Labels = []

for test_signal in test_files:
    test_wav2vec_feature = Wav2Vec2_Features_Extraction(test_signal)
    
    distance_labeld = [(np.sqrt(np.sum((train_wav2vec2_features[i] - test_wav2vec_feature) ** 2)), i) for
                          i in range(len(train_wav2vec2_features))]
