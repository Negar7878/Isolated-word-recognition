...
# Function to extract a fixed number of real cepstral coefficients from a speech signal for each frame
def extract_real_cepstrum(signal, sr, frame_size_ms=30, frame_shift_ms=10, num_ceps=12):
    ...
    cepstral = np.fft.ifft(np.log(np.abs(np.fft.fft(frames, axis=0))), axis=0).real[:, :num_ceps]
    return cepstral
...
train_cepstrals = [extract_real_cepstrum(train_signal, 16000) for train_signal in train_signals]
...
for test_file in test_files:
    ...
    cepstral_test = extract_real_cepstrum(resampled_signal_test, 16000)

    D_ceps_labeld = [(np.sqrt(np.sum((train_cepstrals[i][:min_frame, :] - cepstral_test[:min_frame, :]) ** 2)), i) for
                          i, min_frame in enumerate(map(lambda x: min(x.shape[0], cepstral_test.shape[0]), train_cepstrals))]
